...
export default function CartProvider({children}) {
  ...
  const changeQunatity =  (cartItem, newQuantity) => {
    const { food } = cartItem
  ...
  const addToCart = food => {
    ...
    if(cartItem){
      changeQunatity(cartItem, cartItem.quantity + 1)
    }else{
  ...
  return <CartContext.Provider value={{cart:{items:cartItems, totalPrice, totalCount}, removeFromCart, changeQunatity,addToCart, clearCart}} >
    {children}
